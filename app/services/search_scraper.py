...
def build_rightmove_url(filters: dict[str, str], channel: str = "BUY") -> str:
    ...
    return (
        f"https://www.rightmove.co.uk/property-{preposition}-{action}/find.html"
        + "?"
        + urlencode(filters)
    )
...
def parse_listing_cards(html, filters):
    ...
    for card in result_cards:
        try:
            ...
            room_count_el = card.select_one('span[class^="PropertyInformation_bedroomsCount"]')
            ...
            room_count = room_count_el.text.strip() if room_count_el else None
